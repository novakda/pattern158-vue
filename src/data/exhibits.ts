...
export const exhibits: Exhibit[] = [
  ...
  {
    label: 'Exhibit P',
    client: 'Pattern158 (Personal)',
    date: '2024\u20132026',
    title: 'Career Evidence System \u2014 Reverse Engineering a 28-Year Career from Primary Sources',
    exhibitType: 'engineering-brief',
    contextHeading: 'Context',
    contextText: 'A personal knowledge system built to reverse-engineer a 28-year career narrative from primary sources \u2014 not memory. Anchored by a 35,229-email work archive (2005\u20132022) and supervisor meeting notes (2021\u20132024), the system extracts, classifies, and synthesizes career evidence into public-facing outputs: the Pattern158 brand, a professional website, multiple resumes, and a career vault. Every public claim in those outputs is traceable back to an authoritative source record.',
    sections: [
      {
        type: 'text',
        heading: 'The Core Problem: Claims Without Proof',
        body: 'Career narratives degrade over time. Memory drifts, recency bias amplifies recent work at the expense of older high-impact contributions, and resumes accumulate claims that have lost their source connection. The result is a portfolio that feels authoritative but is, structurally, unsourced.\n\nThe evidence-first approach inverts this: instead of starting from memory and adding citations, start from primary sources and derive the narrative. Every output \u2014 resume bullet, case study summary, brand positioning statement \u2014 should be traceable back to an original document, email, or note. Not as a legal requirement, but as a quality guarantee: if the source doesn\u2019t exist, the claim shouldn\u2019t either.',
      },
      {
        type: 'flow',
        heading: 'The Pipeline: Sources \u2192 Patterns \u2192 Outputs',
        steps: [
          { label: 'Primary Sources', detail: 'Email archive, supervisor meeting notes, performance artifacts, portfolio content' },
          { label: 'Extract', detail: 'Atomic evidence units: decision, deliverable, metric, incident, quote' },
          { label: 'Classify', detail: 'Impact type, capability, certainty level, source pointer' },
          { label: 'Synthesize', detail: 'Repeatable patterns: signature moves, constraints navigated, value delivered' },
          { label: 'Generate', detail: 'Brand, website, resumes, vault, and public-facing artifacts' },
        ],
      },
      {
        type: 'text',
        heading: 'Case Study: Email Archive v8.0 \u2014 Deep Mining',
        body: 'The email archive is a 35,229-email personal work collection spanning 2005\u20132022, supplemented by supervisor meeting notes (2021\u20132024) and website exhibit content. The extraction system uses a non-destructive sidecar architecture: source databases (emails.db, career_data.db) are read-only; all extracted knowledge is written to a separate career-evidence.db. Every extracted fact references its source through an evidence table with a composite key (entity_type, entity_id, source_type, source_id).\n\nMilestone v8.0 \u2014 Deep Mining ran 2026-04-02 through 2026-04-03 (archived 2026-04-07). Two phases, four plans, nine tasks, 78 files changed, 4,225 insertions, ~2,524 LOC Python. All 8 requirements satisfied.',
      },
      {
        type: 'table',
        heading: 'Phase 13 \u2014 Client Folder Mining',
        columns: ['Metric', 'Value'],
        rows: [
          ['Email folders analyzed', '56 customer folders'],
          ['Project records created', '56 project records with evidence references'],
          ['Staging files generated', '56 new project Markdown files promoted draft \u2192 active via idempotent pipeline'],
          ['Evidence graph additions', '56 project records + 57 evidence rows'],
          ['Placeholder replacements', '25 \u201cClient Engagement\u201d entries replaced with specific, evidence-backed projects'],
        ],
      },
      {
        type: 'table',
        heading: 'Phase 14 \u2014 Testimonial Extraction',
        columns: ['Metric', 'Value'],
        rows: [
          ['Candidate identification', '1,412 candidates via FTS5 full-text search + priority folders'],
          ['Existing baseline', '1,170 records (dedup via SequenceMatcher \u2265 0.7)'],
          ['LLM-analyzed candidates', '~1,042 candidates processed'],
          ['Inserted testimonials', '46 extracted \u2192 15 dedup-filtered \u2192 31 inserted'],
          ['Source breakdown', '9 supervisor, 6 client, 16 peer'],
          ['Link coverage', '71% linked to project IDs, 97% linked to skill IDs'],
          ['Guardrails', 'Read-only source DB, confidence threshold, human review required before vault promotion'],
        ],
      },
      {
        type: 'text',
        heading: 'Case Study: Pattern158 Website Build',
        body: 'The Pattern158 website is itself an evidence artifact \u2014 a Vue 3 portfolio built from primary planning documents. The .planning directory serves as the authoritative record of every architectural decision: PROJECT.md (purpose, audiences, constraints, key decisions), RETROSPECTIVE.md (what worked, inefficiencies, patterns established), ROADMAP.md (milestone timeline + phase structure), and STATE.md (accumulated context + decision trail).\n\nShipped across four milestones (v1.0 through v2.1), the site implements an evidence-based portfolio IA with a unified \u201cCase Files\u201d section housing 15 exhibits across two typed layouts. Investigation Reports use an NTSB-style document layout; Engineering Briefs use a constraints-approach-results format. Both types are backed by the same data model: every exhibit has a traceable exhibitType discriminant, all 15 are sourced from the email archive investigation.',
      },
      {
        type: 'table',
        heading: 'Key Architectural Decisions \u2014 Pattern158 Website',
        columns: ['Decision', 'Rationale', 'Outcome'],
        rows: [
          ['Components for consistency, not reuse', 'ADHD-informed: named components reduce cognitive load when scanning templates', 'Pattern held through all 16 phases'],
          ['exhibitType discriminant union', 'Self-documenting type classification; enables type-specific layouts and card styling', 'Clean dispatch pattern, no boolean ambiguity'],
          ['Evidence-backed exhibit content', 'Every public portfolio claim traceable to email archive or meeting notes', 'Zero unsanctioned content added across all milestones'],
          ['TDD for behavioral bugs', 'Writing failing tests before fixing locks the correct semantic mapping', 'RouterLink slot-rendering stub pattern established'],
          ['.planning as primary source', 'Planning docs are machine-readable state; milestone audits verify traceability', 'Three-source cross-reference catches gaps pre-archival'],
        ],
      },
      {
        type: 'text',
        heading: 'The GSD Framework: Making It Repeatable',
        body: 'The system uses a milestone/planning framework called GSD across all projects. Each milestone has machine-readable STATE.md (YAML frontmatter tracking status, progress, and last activity), explicit acceptance criteria in REQUIREMENTS.md, and verified VERIFICATION.md artifacts.\n\nPlans have explicit success criteria stated as \u201cwhat must be TRUE.\u201d Milestone audits cross-reference three sources before archiving: VERIFICATION.md + SUMMARY frontmatter + REQUIREMENTS.md traceability. If a requirement appears satisfied in one source but not traceable in another, the gap surfaces as a gap-closure phase rather than being shipped incomplete. This multi-source verification approach is the same evidence standard applied to the career archive itself.',
      },
      {
        type: 'text',
        heading: 'Reader Starter Kit: A Copyable Mini-System',
        body: 'A minimal version of this approach requires five steps. (1) Gather 20\u201325 primary artifacts \u2014 emails, meeting notes, performance reviews, project docs. (2) Extract ~10 evidence units per artifact: date, context, constraint, outcome, source pointer. (3) Classify each unit: impact type (technical, relational, strategic), capability demonstrated, certainty level (direct quote vs. inferred). (4) Synthesize patterns: what repeats? What constraints do you consistently navigate? What value do you reliably deliver? (5) Generate one output first \u2014 a single resume bullet with a proof pointer, a case study paragraph, or a LinkedIn post linked to one evidence-backed example.\n\nIterate from there. The goal is not a complete system on day one. The goal is a provenance habit: before publishing any professional claim, ask where the source is.',
      },
      {
        type: 'metadata',
        heading: 'System Metadata',
        items: [
          { label: 'Email Archive', value: '35,229 emails (2005\u20132022) + supervisor meeting notes (2021\u20132024)' },
          { label: 'Evidence Database', value: 'SQLite relational evidence graph with entity \u2192 source traceability via composite key' },
          { label: 'Architecture', value: 'Non-destructive sidecar pattern: read-only source DBs, writes only to career-evidence.db' },
          { label: 'Website Build', value: 'Vue 3 + TypeScript + Vite, 4 milestones (v1.0\u2013v2.1), 16 phases, 64 unit tests' },
          { label: 'Outputs', value: 'Pattern158 brand, professional website, multiple resumes, career vault/archive' },
          { label: 'Positioning', value: 'Relational evidence graph today; working toward fuller knowledge graph as evidence coverage solidifies' },
        ],
      },
    ],
    impactTags: ['Evidence-Based Methodology', 'Career Architecture', 'Primary Source Analysis', 'Personal Knowledge Management', 'Python Automation', 'SQLite', 'Vue 3', 'TypeScript', 'Technical Writing', 'Brand Development'],
    exhibitLink: '/exhibits/career-evidence-system',
    isFlagship: true,
    summary: 'A personal knowledge system for reverse-engineering a 28-year career from primary sources \u2014 a 35,229-email archive, supervisor meeting notes, and portfolio content \u2014 into the Pattern158 brand and website. The extraction pipeline uses Python automation against read-only source databases, writes evidence to a relational graph (SQLite), and requires human review before any claim reaches a public-facing output. Two v8.0 deep mining phases extracted 56 project records and 31 testimonials (9 supervisor, 6 client, 16 peer). The Pattern158 website itself is a secondary output: Vue 3 built from planning documents as primary sources, four milestones shipped, all architectural decisions traceable to the same evidence standard.',
    role: 'System architect, engineer, and subject \u2014 the career being reverse-engineered is the same one building the tools',
  },
]
