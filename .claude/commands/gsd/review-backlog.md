...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" phase add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: review backlog — promoted N, removed M" --files .planning/ROADMAP.md
   ```
