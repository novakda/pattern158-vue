...
```bash
INIT=$(node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "/home/xhiris/projects/pattern158-vue/.claude/get-shit-done/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
